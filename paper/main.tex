...
predicting the self-report target \textit{q5}, ``How immersed in your work did
you feel just now?'', and evaluate a
...
for one-class held-out folds. The learned encoders are frozen: only the
prediction head is trained. \placeholder{Insert the common cohort size, the
...
windows from office workers in the TrustMe study. The same labelled windows and subject-
generalisation protocol are used to compare raw signals, handcrafted features,
GazeMAE embeddings, and MOMENT embeddings. The primary target is \textit{q5}
(Engagement): ``How immersed in your work did you feel just now?''

Our research question is: \emph{Which eye-tracking representation most
reliably infers self-reported engagement for unseen office workers?} We make
...
preprocessing; and (3) practical evidence on the out-of-the-box utility of
frozen pretrained encoders relative to raw signals and handcrafted features.
...
window duration, overlap, ordering within participant,
...
held-out subject. Gaze coordinates are first normalized to the display extent
using the recording-resolution metadata. \placeholder{Report the final missing-data policy and
...
\cite{bautista2020gazemae}. Its pretrained encoder is frozen; only the
downstream prediction head is trained.
...
\cite{goswami2024moment}. Its pretrained encoder is likewise frozen; only the
downstream prediction head is trained.
...
Accuracy is the primary metric. We also report balanced accuracy with deltas
over the majority baseline, macro-F1, and AUC. One-class held-out folds are retained; metrics that
...
Insert one main table with representation, model, accuracy ($\Delta$), balanced
accuracy ($\Delta$), macro-F1, and AUC. Deltas are paired against the majority
baseline within each held-out-subject fold before aggregation. Order
representations as raw, handcrafted, GazeMAE, and MOMENT; show the majority
baseline first. State the number of folds on which AUC, balanced accuracy, and
macro-F1 are defined.}
...
\placeholder{Use a gaze-density heatmap on normalized display coordinates and
select one PCA, t-SNE, or UMAP projection coloured by the continuous q5
engagement rating only if it adds an interpretable result; projections are
exploratory, not evidence of predictive separability.}
...
The conclusions are limited to one dataset, one windowing configuration,
self-reported engagement, frozen pretrained checkpoints, and the selected
coordinate-quality and target-binarisation choices.
