...
Handcrafted features come next and improve on the baseline with both classifiers (up to 0.531 accuracy and 0.535 ROC-AUC).
GazeMAE reaches 0.535 accuracy with the random forest and falls to 0.520, below the baseline, with the MLP.
MOMENT is at or below the baseline on every metric, and its ROC-AUC (0.504) is indistinguishable from chance.
...
Most of the structure these representations expose is therefore person-specific, and the frozen encoders appear to smooth it away.
Because the projections are exploratory, their link to the ranking in Table~\ref{tab:main} remains a conjecture.
